...
def main():
    ...
    trainDf = pd.read_csv("Datasets/bank/train.csv", names=columns, na_values='unknown')
    trainDf = trainDf.fillna(trainDf.mode().iloc[0])
    x_train = trainDf.drop('label', axis=1)
    y_train = trainDf['label']

    testDf = pd.read_csv("Datasets/bank/test.csv", names=columns, na_values='unknown')
    ...
    for col in numerical_cols:
        ...
        x_train[col] = (x_train[col] > median_val).astype(int)
        x_test[col] = (x_test[col] > median_val).astype(int)
